...
`source/meme-template-sources.json` 保存 Memegen 与 Imgflip 的目录快照；`source/meme-kym-snapshot.json` 保存 KYM 主档、历史排序、浏览 / 画廊计数、年份及年度 / 近期编辑榜线索。`source/meme-required-entries.json` 是用户点名与基础门槛守门清单；`source/meme-external-benchmark.json` 则只由外部分类页、榜单、专题和具体档案产生候选，本地库只负责覆盖对照。外部基准固定校验中文检索别名、母档 / 变体 / 复合梗关系、来源 URL 与原始状态；中文词仅是检索别名，不宣称官方译名。KYM Submission 只显示为研究中，不得升级为 Confirmed。构建和验证会在任何必收录条目、外部基准家族、来源或别名消失时失败。`tools/build-memes.mjs` 将同一叙事模板的服务别名合并为家族，并把现代研究图缓存为本地 WebP。缓存图不是权利转移，也不进入生产素材池。新增 Meme 必须分别记录原始来源、表达机制、传播 / 复用证据、版权、肖像 / 人格权、商标 / 来源误认和安全改编路线。
