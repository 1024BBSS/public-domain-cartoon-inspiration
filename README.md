...
`memes.html` 是美国 Meme 图谱。当前收录 1,321 个合并后的 Meme 家族：1,271 个现代研究记录、50 个来自现有视觉库的具体公版改编底图；其中 1,052 个带 Know Your Meme 历史传播或编辑证据，286 个首见于 2025–2026，119 个同时满足编辑榜或可量化传播门槛，97 个带 Imgflip 当前模板信号。可按年代、历史传播、当前复用、近年热榜、来源人物 / 作品与权利入口检索。KYM 浏览量、历史排序和画廊数量只作历史传播 / 变体代理；Imgflip 排名与 captions 只作当前平台复用信号；年度榜和周榜只作编辑趋势线索。三者都不是美国人口知名度或授权证明。“新出现”和“已热门”分开计算。关联超级 IP 的调查百分比只属于来源人物 / 作品，不属于 Meme 本身。现代原图默认只供研究；只有标明“公版具体版本”的历史图像进入可复用池，且仍需检查后期表达、商标和商品来源误认。
...
`source/meme-template-sources.json` 保存 Memegen 与 Imgflip 的目录快照；`source/meme-kym-snapshot.json` 保存 KYM 主档、历史排序、浏览 / 画廊计数、年份及年度 / 近期编辑榜线索。`source/meme-required-entries.json` 是用户点名与基础门槛守门清单；`source/meme-external-benchmark.json` 则只由外部分类页、榜单、专题和具体档案产生候选，本地库只负责覆盖对照。`source/meme-topic-associations.json` 把母梗与节日、场景和题材分层关联，当前首个验证集是万圣节；“外部已有变体”“外部题材图式”“结构迁移建议”“公版重绘入口”分开标记，不能把创作推演写成流行事实。外部基准固定校验中文检索别名、母档 / 变体 / 复合梗关系、来源 URL 与原始状态；题材关联固定校验唯一母梗、二创映射、证据解释、来源与权利边界。中文词仅是检索别名，不宣称官方译名。KYM Submission 只显示为研究中，不得升级为 Confirmed。构建和验证会在任何必收录条目、外部基准家族、题材连接、来源或别名消失时失败。`tools/build-memes.mjs` 将同一叙事模板的服务别名合并为家族，并把现代研究图缓存为本地 WebP。缓存图不是权利转移，也不进入生产素材池。新增 Meme 必须分别记录原始来源、表达机制、传播 / 复用证据、版权、肖像 / 人格权、商标 / 来源误认和安全改编路线。

### Meme 低 Token 增量流水线

原始页面先由采集脚本转成 JSON / JSONL；不要把网页全文直接交给模型。`tools/meme-intake.mjs` 会先用来源路径、名称 / 别名和文本相似度与现有家族比对，只把 `new` 与 `possible-duplicate` 写入紧凑审稿批次。已存在记录与纯数值更新不会进入模型上下文。

```bash
# 默认比较 KYM 快照与当前网站；每批 40 条
node tools/meme-intake.mjs prepare

# 也可输入其他采集器生成的 JSON / JSONL
