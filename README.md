...
当前另有 `albums.html`：分层展示公版历史封面、近 70 年需授权经典，以及“鹰翼摇滚”“西部乡村”两条专辑/历史母题脉络，不把现代专辑误作公版素材。
...
`source/cartoon-ip-supplement.json` 与 `source/ghost-commercial-supplement.json` 保存已核验作品、风险边界和 Eagle item ID。`source/album-public-domain.json` 保存公版历史封面；`source/album-license-only.json` 保存现代需授权研究线索；`source/eagle-rock-lineage.json` 与 `source/western-country-lineage.json` 分别保存鹰翼摇滚、西部乡村专题的专辑和历史公版母题。
