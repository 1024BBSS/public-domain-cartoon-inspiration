...
`source/meme-template-sources.json` 保存 Memegen 与 Imgflip 的目录快照；`source/meme-kym-snapshot.json` 保存 KYM 主档、历史排序、浏览 / 画廊计数、年份及年度 / 近期编辑榜线索。`source/meme-required-entries.json` 是抽样遗漏守门清单：固定保留用户点名和基础门槛家族、中文别名、母档 / 变体关系与原始状态；KYM Submission 只显示为研究中，不得升级为 Confirmed。构建和验证会在任何必收录条目或别名消失时失败。`tools/build-memes.mjs` 将同一叙事模板的服务别名合并为家族，并把现代研究图缓存为本地 WebP。缓存图不是权利转移，也不进入生产素材池。新增 Meme 必须分别记录原始来源、表达机制、传播 / 复用证据、版权、肖像 / 人格权、商标 / 来源误认和安全改编路线。
